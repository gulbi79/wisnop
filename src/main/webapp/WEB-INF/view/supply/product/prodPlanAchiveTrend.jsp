...
<%@ include file="/WEB-INF/view/layout/include.jsp" %>
	...
	var prodPlanAchive = {
		...
		_siq    : "supply.product.prodPlanAchiveTrend",
		...
		*/
		salesGrid : {
			...
			initGrid     : function () {
				...
				this.gridInstance.bnFormat = '#,##0.0';
		...
		*/
			...
			var aOption = {
				...
				success : function (data) {
					...
					if (FORM_SEARCH.sql == 'N') {
						prodPlanAchive.salesGrid.dataProvider.clearRows(); //데이터 초기화
				
						//그리드 데이터 생성
						prodPlanAchive.salesGrid.grdMain.cancel();
						
						prodPlanAchive.salesGrid.dataProvider.setRows(data.resList);
						prodPlanAchive.salesGrid.dataProvider.clearSavePoints();
						prodPlanAchive.salesGrid.dataProvider.savePoint(); //초기화 포인트 저장
						gfn_setSearchRow(prodPlanAchive.salesGrid.dataProvider.getRowCount());
						gfn_setRowTotalFixed(prodPlanAchive.salesGrid.grdMain);
		...
		getBucket : function (sqlFlag) {
			...
			if (!sqlFlag) {
				...
				prodPlanAchive.salesGrid.gridInstance.setDraw();
				
				var fileds = prodPlanAchive.salesGrid.dataProvider.getFields();
				...
				prodPlanAchive.salesGrid.dataProvider.setFields(fileds);
	...
	var fn_apply = function (sqlFlag) {
		...
		prodPlanAchive.getBucket(sqlFlag); //2. 버켓정보 조회
    	...
    	FORM_SEARCH.sql        = sqlFlag;
   		...
   		FORM_SEARCH.weekLength = parseInt($("#toWeek").val()) - parseInt($("#fromWeek").val()) + 1; 
   		
   		prodPlanAchive.search();
		prodPlanAchive.excelSubSearch();
	}
