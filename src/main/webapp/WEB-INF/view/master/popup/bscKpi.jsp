...
function fn_setColumns(grid) {
    var columns = [
        {name: "KPI_ID" ,fieldName: "KPI_ID" ,header: {text: '<spring:message code="lbl.kpiId"/>'}, styles: {textAlignment: "near"}, width: 100 },
		{name: "KPI_NM" ,fieldName: "KPI_NM" ,header: {text: '<spring:message code="lbl.kpiNm"/>'}, styles: {textAlignment: "near"}, width: 200 },
    ];
...
function fn_apply() {
	...
	data._mtd = "getList";
	data.tranData = [{outDs:"rtnList",_siq:"master.master.bscOrgMgmtKpi"}];
	var sMap = {
		url: "${ctx}/biz/obj",
        data: data,
...
function fn_save() {
	...
	confirm('<spring:message code="msg.confirmSave"/>', function() {  // 저장하시겠습니까?
		...
		FORM_SAVE.tranData = [{outDs:"saveCnt",_siq:"master.master.bscOrgMgmtKpi", mergeFlag: "Y", grdData : grdData, custDupChkYn : {"insert":"Y"}}];
    	var sMap = {
