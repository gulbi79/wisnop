...
function fn_setColumns(grid) {
    var columns = [
        {name: "DATA_SCOPE_ID" ,fieldName: "DATA_SCOPE_ID" ,header: {text: '<spring:message code="lbl.dataScope"/>'}  , styles: {textAlignment: "near"}, width: 100 },
		{name: "DATA_SCOPE_NM" ,fieldName: "DATA_SCOPE_NM" ,header: {text: '<spring:message code="lbl.dataScopeNm"/>'}, styles: {textAlignment: "near"}, width: 200 },
    ];
...
function fn_apply() {
	...
	data._mtd = "getList";
	data.tranData = [{outDs:"rtnList",_siq:"master.master.bscOrgMgmtScope"}];
	var sMap = {
		url: "${ctx}/biz/obj",
        data: data,
...
function fn_save() {
	...
	confirm('<spring:message code="msg.confirmSave"/>', function() {  // 저장하시겠습니까?
		...
		FORM_SAVE.tranData = [{outDs:"saveCnt",_siq:"master.master.bscOrgMgmtScope", mergeFlag: "Y", grdData : grdData, custDupChkYn : {"insert":"Y"}}];
    	var sMap = {
	...
	<div id="keywordpop" class="popupDv">
		...
		<div class="popCont">
			<div class="srhwrap">
				...
				<ul>
					<li>
					<strong><spring:message code="lbl.level"/></strong>
					<div class="selectBox">
					<input type="text" id="codeNm" name="codeNm" class="ipt" value="${param.DATA_SCOPE_TYPE_NM}" readonly> 
					</div>
					</li>
				</ul>
				</div>
