...
function fn_setData(returnData) {
	if("${param.callback}" != "" && "${param.callback}" != null) {
		...
		if ("${param.callback}" == "gfn_comPopupCallback") {
			eval("opener.${param.callback}")(returnData,"${param.objId}","CUST_CD","CUST_NM");
		} else {
...
function fn_apply() {
	...
	$.each(rows, function(n,v) {
		chkKeyData.push(dataProvider.getValue(v,"CUST_CD"));
		chkData.push(dataProvider.getValue(v,"CUST_CD"));
...
function fn_setColumns(tree) {
    ...
    var columns = [
		{name: "CUST_CD", fieldName: "CUST_CD", header: {text: '<spring:message code="lbl.customer"/>'    }, styles: {textAlignment: "center"}, width: 80 },
		{name: "CUST_NM", fieldName: "CUST_NM", header: {text: '<spring:message code="lbl.customerName"/>'}, styles: {textAlignment: "near"}, width: 242 }
    ];
	...
	if(!gfn_isNull(searchCode)) {
		...
		$.each(arrCode, function(n,v) {
			for (var i=0; i<dataProvider.getRowCount(); i++) {
				if (grdMain.getValue(i, "CUST_CD") == v) {
		...
		<div class="popCont">
			<div class="srhwrap">
				...
				<ul>
					...
					<strong>Name</strong>
					<div class="selectBox">
					<input type="text" id="CUST_NM" name="CUST_NM" value="${param.CUST_NM}" class="ipt">
					</div>
					</li>
				</ul>
				</div>
