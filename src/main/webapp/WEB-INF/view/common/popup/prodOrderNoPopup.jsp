...
function fn_setData(returnData) {
	if("${param.callback}" != "" && "${param.callback}" != null) {
		...
		if ("${param.callback}" == "gfn_comPopupCallback") {
			eval("opener.${param.callback}")(returnData,"${param.objId}","PROD_ORDER_NO","PROD_ORDER_NO");
		} else {
...
function fn_apply() {
	...
	$.each(rows, function(n,v) {
		chkData.push(dataProvider.getValue(v,"PROD_ORDER_NO"));
...
function fn_setColumns(tree) {
	var columns;
	var headTxtCd = '<spring:message code="lbl.prodOrderNo"/>';
	
    columns = [
		{name: "PROD_ORDER_NO", fieldName: "PROD_ORDER_NO", header: {text: headTxtCd}, styles: {textAlignment: "center"}, width: 80  }
    ];
	...
	if(!gfn_isNull(searchCode)) {
		...
		$.each(arrCode, function(n,v) {
			for (var i=0; i<dataProvider.getRowCount(); i++) {
				if (grdMain.getValue(i, "PROD_ORDER_NO") == v) {
...
function fn_getGridData(data) {
	data._mtd = "getList";
	data.tranData = [{outDs:"rtnList",_siq:"common.prodOrderNoPopup"}];
	var sMap = {
        url: "${ctx}/biz/obj",
        data: data,
	...
	<div id="keywordpop" class="popupDv">
		...
		<div class="popCont">
			<div class="srhwrap">
				...
				<ul>
					<li>
					<strong id="sFilter2"><spring:message code="lbl.prodOrderNo"/></strong>
					<div class="selectBox">
					<input type="text" id="PROD_ORDER_NO" name="PROD_ORDER_NO" value="${param.PROD_ORDER_NO}" class="ipt">
					</div>
					</li>
				</ul>
				</div>
