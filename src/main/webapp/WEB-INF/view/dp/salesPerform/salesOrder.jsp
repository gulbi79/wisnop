...
<!-- Sales Order  -->
	...
	var salesOrder = {
		...
		_siq    : "dp.salesPerform.salesOrder",
		...
		initFilter : function() {
			...
			// 달력
			DATEPICKET(null, -3, 6);
			$('#fromCal').datepicker("option", "minDate", new Date().getWeekDay(-106));
			$('#toCal').datepicker("option", "maxDate", new Date().getWeekDay(52, false));
		},
		...
		*/
			...
			var aOption = {
				...
				success : function (data) {
					...
					if (FORM_SEARCH.sql == 'N') {
						salesOrder.salesGrid.dataProvider.clearRows(); //데이터 초기화
				
						//그리드 데이터 생성
						salesOrder.salesGrid.grdMain.cancel();
						
						salesOrder.salesGrid.dataProvider.setRows(data.resList);
						salesOrder.salesGrid.dataProvider.clearSavePoints();
						salesOrder.salesGrid.dataProvider.savePoint(); //초기화 포인트 저장
						gfn_setSearchRow(salesOrder.salesGrid.dataProvider.getRowCount());
						gfn_setRowTotalFixed(salesOrder.salesGrid.grdMain);
		...
		getBucket : function (sqlFlag) {
			var ajaxMap = {
				...
				sqlId    : ($(':radio[name=vhWeekType]:checked').val() == "SW" ? ["bucketFullWeek"] : ["bucketWeek"])
			}
			...
			if (!sqlFlag) {
				...
				salesOrder.salesGrid.gridInstance.setDraw();
				
				var fileds = salesOrder.salesGrid.dataProvider.getFields();
				...
				salesOrder.salesGrid.dataProvider.setFields(fileds);
			}
		}
	};
	...
	var fn_apply = function (sqlFlag) {
		...
		salesOrder.getBucket(sqlFlag); //2. 버켓정보 조회
    	...
    	FORM_SEARCH.sql        = sqlFlag;
   		...
   		FORM_SEARCH.menuId = "salesOrder";
   		
		salesOrder.search();
		salesOrder.excelSubSearch();
	}
