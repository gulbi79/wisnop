...
.modal_header > .pupupInfo_etc > .writer_container > #popupWriterTeam{
...
.modal_header > .pupupInfo_etc > .postingPeriod_container > #popupStartDate{
	padding-top:2px;
 	padding-left:0;
 	padding-right:0;
 	padding-bottom:2px;
...
.modal_header > .pupupInfo_etc > .postingPeriod_container > #popupCloseDate{
	padding-top:2px;
 	padding-left:0;
 	padding-right:0;
 	padding-bottom:2px;
...
<body>
	...
	<div class="modal">
		<div class="modal_header">
			...
			<div class="pupupInfo_etc">
				<div class="writer_container">
					<p><spring:message code="lbl.writer"/>:</p>
					<input id="popupWriter"  type="text" readonly="readonly" size="4" />
					<input id="popupWriterTeam"  type="text" readonly="readonly" size="7" />
				</div>
					
				<div class="postingPeriod_container">
					<p><spring:message code="lbl.postingPeriod"/>:</p>
				<input id="popupStartDate" type="text" readonly="readonly" size="10" /><p>~</p><input id="popupCloseDate" type="text" readonly="readonly" size="10" />
				</div>
			</div>
		
			<div class = "attachedFile" id="attachedFile"></div>	
		</div>
		...
		<div class="modal_footer">
			<div class="modal_Btn_Container">
				...
				<div class="btn_box">
					<button type="button"   onclick="fn_close_noMore();" class="button">더이상 열지않기</button>
				</div>
				<div class="btn_box">
					<button type="button" onclick="fn_close_today();" class="button">오늘하루 열지않기</button>
				</div>
	...
	$("document").ready(function () {
		...
		function fn_init_TextInfo(){
			...
			$('#popupTitle').val(FORM_SEARCH.rtnList[0].TITLE);
			$('#popupWriter').val(FORM_SEARCH.rtnList[0].USER_ID);
			$('#popupWriterTeam').val('('+FORM_SEARCH.rtnList[0].TEAM_CD+')');
			
			$('#popupStartDate').val(FORM_SEARCH.rtnList[0].START_DATE);
			$('#popupCloseDate').val(FORM_SEARCH.rtnList[0].CLOSE_DATE);
		...
		function fn_loadNoticeBoardPopupContents(){
			...
			FORM_SEARCH.NOTICE_ID = "${param.NOTICE_ID}";
			FORM_SEARCH.FILE_NO = fileNo
			FORM_SEARCH._mtd = "getList";
			FORM_SEARCH.tranData = [
				{outDs:"rtnList",_siq:"admin.noticeBoardPopupContent"},
	...
	//fn_close_today()
		...
		function fn_close_today()
		{
			
			setCookie("${param.NOTICE_ID}","end",1);
